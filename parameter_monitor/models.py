...
class NetworkStat(models.Model):
    ...
    def __str__(self):
        return "| Reading date: {}\n | Internet [ Upload: {} | Download {} ]\n".format(
            self.reading_timestamp.strftime("%m/%d/%Y, %H:%M:%S"), self.download, self.upload
        )


class GeneralSystemParameter(models.Model):
    reading_timestamp = models.DateTimeField(default=now)
    ...
    cpu_curr_freq = models.FloatField(default=0.0)
    cpu_max_freq = models.FloatField(default=0.0)

    memory_total = models.FloatField(default=0)
    memory_used = models.FloatField(default=0)
    memory_available = models.FloatField(default=0)
    ...
    def __str__(self):
        return "| Reading date: {}\n | CPU [ Load: {} | Temp (C) : {} ]\n | Memory [ Percentage Free: {}% ] | Disk [ Percentage Free: {}% ]".format(
            self.reading_timestamp.strftime("%m/%d/%Y, %H:%M:%S"),
            self.cpu_percent_15_min,
            self.cpu_temperature,
            self.memory_free_percentage,
            self.disk_free_percentage,
        )
